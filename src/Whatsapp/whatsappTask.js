...
export default function WhatsappTask() {
  ...
  const api = async (value) => {
    await axios
      .get(common_Link + "/task/statuswhatsapp", {
        headers: {
          ...
          {loading?<BeatLoader color="lightblue" />:task.map((props) => {
            return (
              ...
              >
                ...
                await axios.patch(common_Link+'/task/updatewhatsapp',{
                           '_id':props._id,  
                           'projectname':localStorage.getItem('projectname'),
                           'assignedTo':props.assignedTo,
              ...
              >
                ...
                <br/>
                 <button class='btn-danger' onClick={async()=>{
                      await axios.delete(common_Link+'/task/deletewhatsapp',{data:{
                        '_id':props._id
                      }}).then(resp=>{
                        if(resp.status==200){
